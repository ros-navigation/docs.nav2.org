...
In this part of the guide, we discuss how your robot can utilize different planner and controller algorithms to complete navigation tasks. We will discuss some of the available algorithm plugins you can select from based on your robot type and environment. Take note that this guide only sets up a few prerequisites that we will need for the full navigation stack. 

Planner and Controller Servers
******************************
Navigation algorithms are implemented in Nav2 through the use of plugins running on ROS action servers called planners, controllers and recovery servers. For this section, we discuss the planner and controller servers which are the heart of the navigation stack. These servers may implement one or more algorithms plugins each with its own configuration tailored for a specific action or robot state. We talk about how our robot can select between these different plugins and adjust its behavior in a future guide. 
...
The controller server generates the appropriate control efforts needed for a robot to complete a task in its local environment. These tasks may include but are not limited to: following a path generated by the planner server, avoiding dynamic obstacles along this path, and even charging at a docking station. 
