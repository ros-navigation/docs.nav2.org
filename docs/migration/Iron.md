...
A new metapackage exists in: [https://github.com/open-navigation/opennav_docking](https://github.com/open-navigation/opennav_docking) which contains complete automatic charging dock framework, BT nodes, plugins, and demos.
...
See [Using Docking Server](../tutorials/docs/using_docking.md#docking-tutorial) for a tutorial on using this new capability! Thanks to NVIDIA for sponsoring this package!
...
[PR #3612](https://github.com/ros-navigation/navigation2/pull/3612) adds a new MaxWithoutUnknownOverwrite option to combination_method parameter in Voxel and Obstacle Layers. This can be used to make sure that the static map is the dominant source of information, and
...
In this [PR #3871](https://github.com/ros-navigation/navigation2/pull/3871) the type of duration variable in wait_action node is changed from int to double, which allows you to use floating values for wait_action.
...
See [Graceful Controller](../configuration/packages/configuring-graceful-motion-controller.md#configuring-graceful-motion-controller) for more information.
...
[PR #4441](https://github.com/ros-navigation/navigation2/pull/4441) adds a new parameter use_cancel_deceleration for the regulated pure pursuit controllers. This parameter enables the controller to use a constant deceleration to stop the robot gracefully instead of stopping immediately when a goal is canceled.
