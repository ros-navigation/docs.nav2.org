...
**Note:** Currently, backward_ros does not work with ComposedNodes due to missing support in rclcpp. Until this [issue](https://github.com/ros2/rclcpp/issues/2396) is resolved, launching the Nav2 stack with composition disabled (e.g. with a use_composition:=False launch argument) is required to get automatic backtraces.
