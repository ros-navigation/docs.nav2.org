...
In order to enable Keepout Filter in your configuration, both servers should be enabled as a lifecycle nodes in Python launch-file. The launch file below does not start a Lifecycle Manager of its own: `bringup_launch.py` adds the nodes returned by its `get_lifecycle_nodes()` function to `lifecycle_manager_nav2`. It is also possible to add them as Composition Nodes to your Navigation Component Container, which might look as follows:
