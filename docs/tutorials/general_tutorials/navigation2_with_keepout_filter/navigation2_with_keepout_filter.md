...
Create a new image with a PGM/PNG/BMP format: copy [depot.pgm](https://github.com/ros-navigation/navigation2/blob/lyrical/nav2_bringup/maps/depot.pgm) main map which will be used in a world simulation from a `Nav2` repository to a new `depot_keepout.pgm` file.
...
<figure markdown="span">
  ![](assets/drawing_keepout_mask.png){ width="500px" }
</figure>

After all keepout areas will be filled, save the `depot_keepout.pgm` image.

Like all other maps, filter mask should have its own YAML metadata file. Copy [depot.yaml](https://github.com/ros-navigation/navigation2/blob/lyrical/nav2_bringup/maps/depot.yaml) to `depot_keepout.yaml`. Open `depot_keepout.yaml` and correct `image` field to a newly made PGM mask:
...
Ready-to-go standalone Python launch-script, YAML-file with ROS parameters and filter mask example for Keepout Filter could be found in [nav2_bringup](https://github.com/ros-navigation/navigation2/tree/lyrical/nav2_bringup) directory. To run the demo, simply launch as follows:
...
<figure markdown="span">
  ![](assets/keepout_global.png){ height="400px" }
</figure>
