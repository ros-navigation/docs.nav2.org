...
This package is a generalized EKF and UKF solution to state estimation of potentially many different topics, publishing at different rates, of different types. If you’re unfamiliar with `robot_localization` checkout our [First-Time Robot Setup Guide][first-time-robot-setup-guide]’s Odometry page for basic information and the [package’s extensive documentation](http://docs.ros.org/en/noetic/api/robot_localization/html/index.html).
