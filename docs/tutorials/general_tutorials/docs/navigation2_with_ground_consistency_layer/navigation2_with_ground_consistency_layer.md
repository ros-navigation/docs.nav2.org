# Ground Terrain Segmentation using 3D Lidar { #ground-terrain-segmentation-using-3d-lidar }

## Overview

In this tutorial, we demonstrate a terrain-aware costmap layer for Nav2 which uses 3D lidar ground segmentation. The
[ground consistency costmap layer](https://github.com/dfki-ric/nav2_ground_consistency_costmap_plugin) allows users
...
<figure markdown="span">
  ![Ground Consistency Layer Demo](images/ground_consistency_layer.gif){ title="Ground Consistency Layer Demo" }
...
This tutorial requires ROS 2 Jazzy and Nav2. If you don't have Nav2 already installed, check out our [Quickstart][quickstart] page before moving forward.
...
<figure markdown="span">
  ![Raw Input Point Cloud](images/raw_points.png){ title="Raw Input Point Cloud"}
  <figcaption>Raw Input Point Cloud</figcaption>
</figure>

<figure markdown="1">
  ![Segmented Ground and Obstacle Points](images/segmented_points.png){ title="Segmented Ground and Obstacle Points"}
...
For a complete and exhaustive reference of all available parameters, consult the [plugin documentation](https://github.com/dfki-ric/nav2_ground_consistency_costmap_plugin).

## Use Cases

| **Use Case**                | **Challenge**                                                                                                 | **Solution**                                                                                                                                                                                                                  |
|-----------------------------|---------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Uneven Terrain & Slopes     | Standard costmaps mark slopes and elevation changes as obstacles, blocking navigation on traversable terrain  | Ground Consistency uses terrain-relative heights, distinguishing navigable slopes from actual blocking obstacles. Lower decay values allow quick response to terrain changes.                                                 |
| Navigation Under Structures | Overhead structures (bridges, tree canopies) are incorrectly marked as blocking                               | Set `maximum_height_filter` to filter out overhead obstacles. Also ensure `robot_height` is accurately configured, as it's used to determine which obstacles actually block the robot when heights of overhead obstacles vary |
| Cluttered Environments      | Branches, leaves, and small debris create false positive obstacles                                            | Use `min_clearance` to filter out debris below robot height; only actual blocking-height obstacles are marked                                                                                                                 |
| Dynamic Environments        | Sensor noise causes flickering costmaps with frequent occupation/free state changes                           | Evidence accumulation and decay smooth the costmap over time, creating temporal stability                                                                                                                                     |
...
1. Demonstration of ground consistency layer with [Arter Excavator](https://robotik.dfki-bremen.de/en/research/robot-systems/arter) at the [Robotics Innovation Center](https://robotik.dfki-bremen.de/en/startpage) (DFKI) in Bremen, Germany:
    ...
    <figure markdown="span">
      ![Ground Consistency Layer Use Cases](images/arter.gif){ width="700px" title="Ground Consistency Layer Use Cases"}
...
2. Demonstration in [BotanicGarden](https://github.com/robot-pesg/BotanicGarden) dataset:
    ...
    <figure markdown="span">
      ![Ground Consistency Layer Use Cases](images/botanic_garden.gif){ width="700px" title="Ground Consistency Layer Use Cases"}
      <figcaption></figcaption>
    </figure>
...
3. Demonstration in [CitrusFarm](https://github.com/UCR-Robotics/Citrus-Farm-Dataset) dataset:
    ...
    <figure markdown="span">
      ![Ground Consistency Layer Use Cases](images/citrus_farm.gif){ width="700px" title="Ground Consistency Layer Use Cases"}
      <figcaption></figcaption>
    </figure>
...
<figure markdown="span">
  ![Gazebo Baylands world](images/husky_in_baylands.png){ width="700px" title="Gazebo Baylands world"}
  <figcaption></figcaption>
</figure>
...
<figure markdown="span">
  ![RViz2 window showing costmap layers](images/rviz_window.png){ width="700px" title="RViz2 window showing costmap layers"}
  <figcaption></figcaption>
</figure>
...
<figure markdown="span">
  ![Robot navigating through terrain with ground consistency layer](images/robot_in_action.png){ width="700px" title="Robot navigating through terrain with ground consistency layer"}
  <figcaption></figcaption>
</figure>
