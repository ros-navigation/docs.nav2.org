...
!!! info "See also"
    ...
    For a more in-depth discussion on Navigation Servers, we suggest to have a look at the [Navigation Servers](/concepts/index.md#navigation-servers) section under the Navigation Concepts category.
...
As mentioned, the planner server may utilize plugins that work on the grid space such as the `NavFn Planner`, `Smac Planner 2D`, and `Theta Star Planner`. The [NavFn planner](/configuration/packages/configuring-navfn.md) is a navigation function planner that uses either Dijkstra or A\*.  Next, the [Smac 2D planner](/configuration/packages/configuring-smac-planner.md) implements a 2D A\* algorithm using 4 or 8 connected neighborhoods with a smoother and multi-resolution query. Lastly, the [Theta Star planner](/configuration/packages/configuring-thetastar.md) is an implementation of Theta\* using either line of sight to create non-discretely oriented path segments.
...
Another example for this section is the [Regulated Pure Pursuit controller (RPP)](/configuration/packages/configuring-regulated-pp.md) . This controller implements a variant of the pure pursuit algorithm with added regulation heuristic functions to manage collision and velocity constraints. This variation is implemented to target the needs of service or industrial robots and is suitable for use with **differential, ackermann, and legged** robots.
