...
In [PR #5264](https://github.com/ros-navigation/navigation2/pull/5264), option to use [point_cloud_transport](https://github.com/ros-perception/point_cloud_transport) has been added.
...
In [PR #5781](https://github.com/ros-navigation/navigation2/pull/5781), a new parameter inscribed_obstacle_cost_value is added to fix the issue where Costmap2DPublisher maps INSCRIBED_INFLATED_OBSTACLE (253) to value 99 in OccupancyGrid, but StaticLayer incorrectly converts it back to 251 instead of 253.
...
[PR #5933](https://github.com/ros-navigation/navigation2/pull/5933) refactors the Inflation layer to leverage OpenMP for parallel processing, significantly improving performance in large maps.
...
[PR #6036](https://github.com/ros-navigation/navigation2/pull/6036) enhances MPPI trajectory visualization with per-critic cost coloring.
...
[PR #6000](https://github.com/ros-navigation/navigation2/pull/6000) changes the cost function formulation in *nav2_constrained_smoother*. Weights for the constrained smoother may need to be adjusted as a result.
...
[PR #6062](https://github.com/ros-navigation/navigation2/pull/6062) converts `nav2_loopback_sim` from Python to C++ for improved performance and consistency with the rest of the Nav2 stack. This results in roughly a 2x improvement in CPU consumption.
...
[PR #6076](https://github.com/ros-navigation/navigation2/pull/6076) adds support for plugin-based configuration of motion models in MPPI.
...
## Adaptive tolerance goal checker { #kilted-to-l-adaptive-tolerance-goal-checker }

[PR #6052](https://github.com/ros-navigation/navigation2/pull/6052) adds a new adaptive tolerance goal checker.
...
[PR #6140](https://github.com/ros-navigation/navigation2/pull/6140) extends the below costmap clearing services to allow optionally clearing a subset of selected costmap plugins.
...
[PR #6118](https://github.com/ros-navigation/navigation2/pull/6118) adds a new asymmetric inflation field.
...
[PR #6279](https://github.com/ros-navigation/navigation2/pull/6279) adds a dedicated goal validation callback to Nav2 action servers.
...
[PR #6104](https://github.com/ros-navigation/navigation2/pull/6104) adds a new `ZoneParameterFilter` costmap filter that sets ROS parameters on other nodes based on the robot's position using a filter mask. Each mask value selects a declared state carrying parameter setpoints, for example a lower `FollowPath.max_vel_x` inside a snow zone. Leaving all zones, or entering a zone with mask value `0`, restores the declared `nominal_defaults`. Every state transition is published as `std_msgs/UInt8` on the `state_event_topic`. See the [Zone Parameter Filter Parameters][zone-parameter-filter-parameters] configuration page for the parameters and an example, and the [Navigating with Zone Parameter Overrides][navigating-with-zone-parameter-overrides] tutorial.

<figure markdown="span">
  ![A single zone raising inflation_radius on the local and global costmap nodes, with a panel tracking both parameter values](../../configuration_guide/core_servers/costmap_2d/costmap_filters/zone_parameter_filter/assets/zone_parameter_filter_demo.gif){ width="960px" title="A single zone raising inflation_radius on the local and global costmap nodes, with a panel tracking both parameter values"}
</figure>
