...
Source code and `README` with design, explanations, and metrics can be found on [Github](https://github.com/ros-navigation/navigation2/tree/jazzy/nav2_smac_planner). A brief explanation can be found below, but the `README` contains the most detailed overview of the framework and planner implementations.
...
<figure markdown="span">
  ![Paths generated by the Smac Planners](images/3planners.png){ title="Paths generated by the Smac Planners" }
  <figcaption>2D A* (Panel 1),  Hybrid-A* (Panel 2), State Lattice (Panel 3)</figcaption>
</figure>
