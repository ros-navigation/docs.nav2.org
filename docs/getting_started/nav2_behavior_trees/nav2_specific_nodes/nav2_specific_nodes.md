...
The full list of custom BT nodes can be found in the [nav2_behavior_tree plugins folder](https://github.com/ros-navigation/navigation2/tree/lyrical/nav2_behavior_tree/plugins).
The [configuration guide][behavior-tree-xml-nodes] can also be quite useful.
...
For additional details regarding the `PipelineSequence` please see the [PipelineSequence configuration guide][pipeline-sequence].
...
For additional details regarding the `RecoveryNode` please see the [RecoveryNode configuration guide][recovery-node].
...
For additional details regarding the `NonblockingSequence` please see the [NonblockingSequence configuration guide][nonblocking-sequence].
