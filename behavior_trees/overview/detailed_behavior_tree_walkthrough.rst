...
- A ``Fallback`` node first checks whether planner or controller recoveries might help resolve the issue. If either reutrns ``SUCCESS``, the fallback succeeds and the sequence proceeds to the next step.
